...
```{r}
load('Merged_pigment_data.RData')
...
```{r AQ_SQ_results, fig.width=12, fig.height=10, echo=F}
...
# AQ Vietnam is an outlier so we use the raw clinical data
mod_AQV_bk = glm(outcome ~ as.numeric(prop_pigneut>0.05) + log(lactate), 
                 family='binomial', 
                 data = pigmt_data[aqViet_ind, ])
summary(mod_AQV_bk)$coefficients
