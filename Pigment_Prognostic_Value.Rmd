...
library(dplyr)
library(missForest)
library(bootpredictlme4) ## bootstrap uncertainty for mixed-models
library(plotrix)
library(mosaic)
...
```{r Fig1, fig.width=12, fig.height=10, echo=F}
par(las=1, bty='n', cex.lab=1.9, cex.axis=1.7, family='serif')
...
mtext(text = paste0('(n=',sum(notna_para&(sq_ind|aqViet_ind)),')'),
      line = 1, side = 3, adj = 1, cex=1.3)
mtext(text = 'a', line = 1,side = 3, adj = 0, cex=1.3)
par(mar = c(5,6,0,4))
...
axis(1, at = 2:6, labels = c(expression(10^2),
                             ...
                             expression(10^5),
                             expression(10^6)))
text(x = 3.5, y=40, labels='Asian adults\nand children',cex=2)
...
mtext(text = paste0('(n=',sum(notna_para&aq_ind),')'),
      line = 1, side = 3, adj = 1, cex=1.3)
mtext(text = 'd', line = 1,side = 3, adj = 0, cex=1.3)
par(mar = c(5,6,0,4))
...
axis(1, at = 2:6, labels = c(expression(10^2),
                             ...
                             expression(10^5),
                             expression(10^6)))
text(x = 3.5, y=17.5, labels='African children',cex=2)
polygon(x = c(xs, rev(xs)),
        y = 100*inv.logit(c(my_preds$fit+1.96*my_preds$se.fit,
                            rev(my_preds$fit-1.96*my_preds$se.fit))),
        border = NA, col = adjustcolor('grey',alpha.f = .3))
...
mtext(text = paste0('(n=',sum(notna_neut&(sq_ind|aqViet_ind)),')'),
            line = 1, side = 3, adj = 1, cex=1.3)
mtext(text = 'b', line = 1,side = 3, adj = 0, cex=1.3)
par(mar = c(5,6,0,4))
...
plot(log10(xs+lambda_pigment), 
     ...
     type='l',lwd=3)
text(x = -1.65, y=40, labels='Asian adults\nand children',cex=2)
...
mtext(text = paste0('(n=',sum(notna_neut&(aq_ind)),')'),
            line = 1, side = 3, adj = 1, cex=1.3)
mtext(text = 'e', line = 1,side = 3, adj = 0, cex=1.3)
par(mar = c(5,6,0,4))
...
text(x = -1.65, y=17.5, labels='African children',cex=2)
abline(v=log10(my_v_ticks/100+lambda_pigment), lty='dotted',  col='lightgray')
axis(1, at = log10(c(0,1,2,5,10,20)/100+lambda_pigment), 
     labels = c(0,1,2,5,10,20))
polygon(x = log10(c(xs, rev(xs))+lambda_pigment),
...
mtext(text = paste0('(n=',sum(notna_mono&(sq_ind|aqViet_ind)),')'),
            line = 1, side = 3, adj = 1, cex=1.3)
mtext(text = 'c', line = 1,side = 3, adj = 0, cex=1.3)
...
mtext(text = paste0('(n=',sum(notna_mono&(aq_ind)),')'),
           line = 1, side = 3, adj = 1, cex=1.3)
mtext(text = 'f', line = 1,side = 3, adj = 0, cex=1.3)
...
```{r meta_params, echo=FALSE}
myrightcols = c('effect')
mymethod = 'GLMM'
```
...
```{r PMN_5percent_children, fig.height=7}
...
```{r PMN_5percent_adults, fig.height=4}
ind_not_na = !is.na(pigmt_data$prop_pigneut) &
  !is.na(pigmt_data$outcome) & !is.na(pigmt_data$age) &
  pigmt_data$age >15
ind_pos = pigmt_data$prop_pigneut > 0.05
...
```{r pigmented_monocytes_5percent, fig.height=7}
